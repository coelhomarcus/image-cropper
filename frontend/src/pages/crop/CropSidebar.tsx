import { X, RotateCcw, ChevronDown, Settings, Crop as CropIcon, Loader2 } from "lucide-react";
    ...
    <>
      ...
      `}>
        ...
        <div className="lg:hidden flex items-center justify-between p-4 border-b border-[#2d2e3a]">
          <span className="text-sm font-medium text-[#f3f4f6]">Configurações</span>
          <button onClick={onClose} className="p-1 text-[#6b7280] hover:text-[#f3f4f6]">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="lg:hidden p-4 border-b border-[#2d2e3a]">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`px-2 py-1 rounded text-xs font-medium ${isGif ? "bg-purple-500/20 text-purple-400" : "bg-blue-500/20 text-blue-400"}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          {/* Crop Rectangle */}
          <section>
            <h3 className="text-[#f3f4f6] font-medium text-sm mb-3">Tamanho do Recorte</h3>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[#6b7280] text-xs mb-1.5">Largura</label>
                <input 
                  type="number" 
              ...
              <div>
                <label className="block text-[#6b7280] text-xs mb-1.5">Altura</label>
                <input 
                  type="number" 
          ...
          {/* Aspect Ratio */}
          <section>
            <h3 className="text-[#f3f4f6] font-medium text-sm mb-3">Proporção</h3>
            <select 
            ...
            >
              {(Object.keys(ASPECT_RATIOS) as AspectRatioKey[]).map((key) => (
                <option key={key} value={key}>{key === "Livre" ? "Livre" : key}</option>
              ))}
            </select>
          ...
          {/* Crop Position */}
          <section>
            <h3 className="text-[#f3f4f6] font-medium text-sm mb-3">Posição do Recorte</h3>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[#6b7280] text-xs mb-1.5">Posição (Y)</label>
                <input type="number" value={Math.round(cropY)} readOnly className="dark-input w-full opacity-60" />
              </div>
              <div>
                <label className="block text-[#6b7280] text-xs mb-1.5">Posição (X)</label>
                <input type="number" value={Math.round(cropX)} readOnly className="dark-input w-full opacity-60" />
              </div>
            </div>
          </section>
          ...
          {isGif && (
            <section className="border-t border-[#2d2e3a] pt-4">
              ...
              >
                ...
                <ChevronDown className={`w-4 h-4 ml-auto transition-transform ${showGifSettings ? "rotate-180" : ""}`} />
              </button>
              {showGifSettings && (
                <div className="mt-3">
                  <label className="block text-[#6b7280] text-xs mb-2">Cores: {gifSettings.colors}</label>
                  <input 
                    type="range" 
                    min="16" 
                    max="256" 
